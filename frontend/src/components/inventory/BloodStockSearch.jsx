...
import { Droplets, Search, ExternalLink, ArrowRight } from 'lucide-react'
...
export function BloodStockSearch({ variant = 'full' }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {submitted && availabilityQuery.isLoading && (
        <Card>
          <CardContent className="p-0">
            <TableSkeleton rows={variant === 'compact' ? 4 : 6} />
          </CardContent>
        </Card>
      )}
      ...
      {variant === 'compact' && submitted && availabilityQuery.data && (
        <div className="flex justify-end">
          <Link
            to="/blood-stock"
            className="inline-flex items-center gap-1.5 text-sm font-medium text-brand-600 hover:underline"
          >
            View full search <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      )}
    </div>
...
function ResultsSection({ data, labels, compact }) {
  const { count, results } = data
  const displayed = compact ? results.slice(0, 6) : results
